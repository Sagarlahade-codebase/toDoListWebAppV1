...
import net.javaguides.todoapp.web.*;
...
public class TodoDaoImpl implements TodoDao {
	...
	private static final String INSERT_TODOS_SQL = "INSERT INTO todos"
			+ "  (title, username, description, target_date,  is_done,priority) VALUES " + " (?, ?, ?, ?, ?,?);";

	private static final String SELECT_TODO_BY_ID = "select id,title,username,description,target_date,is_done,priority from todos where id =?";
	private static final String SELECT_ALL_TODOS = "select * from todos";
	private static final String DELETE_TODO_BY_ID = "delete from todos where id = ?;";
	private static final String UPDATE_TODO = "update todos set title = ?, username= ?, description =?, target_date =?, is_done = ?,priority=?  where id = ?;";
	...
	public Todo selectTodo(long todoId) {
		...
		try (Connection connection = JDBCUtils.getConnection();
			...
			while (rs.next()) {
				...
				todo = new Todo(id, title, username, description, targetDate, isDone,priority);
			}
		} catch (SQLException exception) {
			JDBCUtils.printSQLException(exception);
		}
	...
	@Override
	public TreeSet<Todo> selectAllTodos() {

		// using try-with-resources to avoid closing resources (boiler plate code)
		TreeSet<Todo> todos = new TreeSet<>(new MyComparator());
		...
		try (Connection connection = JDBCUtils.getConnection();
			...
			while (rs.next()) {
				...
				Integer priority=rs.getInt("priority");
				boolean isDone = rs.getBoolean("is_done");
				todos.add(new Todo(id, title, username, description, targetDate, isDone,priority));
			}
		} catch (SQLException exception) {
			JDBCUtils.printSQLException(exception);
		}
	...
	public boolean updateTodo(Todo todo) throws SQLException {
		...
		try (Connection connection = JDBCUtils.getConnection();
			...
			statement.setLong(6, todo.getPriority());
